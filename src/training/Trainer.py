...
class Trainer:
    ...
    priority_mask_mode='field_strength',min_priority=0.1, physically_informed_loss=False,alpha=0.1):
        ...
        self.physically_informed_loss = physically_informed_loss
        self.alpha = alpha
    ...
    def run_epoch(self, train, dataloader):
        ...
        for x, y in dataloader:
            ...
            if self.physically_informed_loss:
                shifted_pred_xMinus1 = torch.nn.functional.pad(pred,(0,0,1,1))[:,:,:x_lim,:]
                shifted_pred_xPlus1 = torch.nn.functional.pad(pred,(0,0,1,1))[:,:,2:,:]
                shifted_pred_yMinus1 = torch.nn.functional.pad(pred,(1,1))[:,:,:,:y_lim]
                shifted_pred_yPlus1 = torch.nn.functional.pad(pred,(1,1))[:,:,:,2:]
                continuity_values = (shifted_pred_xPlus1[:,0,:,:] + shifted_pred_yPlus1[:,1,:,:] - shifted_pred_xMinus1[:,0,:,:] - shifted_pred_yMinus1[:,1,:,:])/4
                continuity_loss =  (continuity_values*continuity_values).sum()
                
                #print(continuity_loss)
                loss += self.alpha * continuity_loss
